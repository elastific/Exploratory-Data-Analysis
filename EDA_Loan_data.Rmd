...
> **Tip**: In this section, we perform some preliminary exploration of
our dataset. we run some summaries of the data and create univariate plots to
...
current. About 200 thousand are fully paid and about 45 thousand loans are charged
off. 
...
majority of the loans are in Grades A, B, and C. The interesting observation here
was that as we move from grade A to Grade G, the loan amount distribution becomes
more homogeneous.
...
```{r}
annual_inc.new$Income_to_loan_ratio <- annual_inc.new$annual_inc / !is.na(annual_inc.new$loan_amnt)
ggplot(data = annual_inc.new, aes(Income_to_loan))+
  geom_density(fill= 'gray')+
...
different Home Ownership status. More than 443 thousand loans were granted to those
with a mortgaged home, but only 87 thosand to those who own their home. A clear
difference between the annual inmcome of those with different home ownership is 
also shown.
